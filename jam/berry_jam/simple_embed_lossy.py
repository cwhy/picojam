...
def init() -> Tuple[Dict[str, Any], Dict[str, Array]]:
    ...
    config = {
        ...
        "target_loss_var_start": 0.1,
        "target_loss_var_decay": 0.9999,
    }
    ...
    params = {
        ...
        'loss_embed_period': jax.random.normal(next(key_gen).get(), (embed_dim,)),
        'loss_embed_phase': jax.random.normal(next(key_gen).get(), (embed_dim,)) * 2 * jnp.pi,
        'loss_embed_bias': jnp.zeros((embed_dim,), dtype=jnp.float32),
        'bias': jnp.zeros(1, dtype=jnp.float32),
    }
    return config, params, key_gen

def get_loss_embeddings(params: Dict[str, Array], loss: float) -> Array:
    """Get embeddings for loss"""
    return jnp.sin(loss * params['loss_embed_period'] + params['loss_embed_phase']) + params['loss_embed_bias']
...
def get_pixel_values_1(params: Dict[str, Array], coords: Iterable[int], img_idx: int, loss: float=0) -> Array:
    ...
    loss_embeddings = get_loss_embeddings(params, loss)
    return jax.nn.sigmoid((pos_embeddings * img_embeddings) @ loss_embeddings + params['bias'])   

def get_image(params: Dict[str, Array], img_idx: int, loss: float=0) -> Array:
    """Get the full image for a given image index"""
    coords = create_coordinate_indices(*img_dims)
    pixel_values = jax.vmap(get_pixel_values_1, in_axes=(None, 0, None, None))(params, coords, img_idx, loss)
    return pixel_values.reshape(img_dims)

def loss_batch_1(params: Dict[str, Array], img_idx: int, target_img: Array, target_loss: float) -> Array:
    """Compute the loss for one image"""
    return jax.nn.relu(jnp.mean((get_image(params, img_idx, target_loss) - target_img) ** 2) - target_loss)

def loss_batch(params: Dict[str, Array], img_idx_batch: Array, target_batch: Array, target_loss: Array) -> Array:
    ...
    return loss_fn(params, img_idx_batch, target_batch, target_loss)  



@partial(jax.jit, static_argnums=(0, ))
def train_step(optimizer: optax.GradientTransformation, params: Dict[str, Array], opt_state: Any, indices: Array, batch_images: Array, batch_loss: Array) -> Tuple[optax.Params, optax.OptState, Array]:

    def _loss_batch(p):
        return jnp.mean(loss_batch(p, indices, batch_images, batch_loss))
...
def test_train_step_100(params: Dict[str, Array], optimizer: optax.GradientTransformation, X: Array) -> Array:
    ...
    batch_loss = jnp.array([0, 1, 2])
    for epoch in range(1000):
        params, opt_state_, loss_value = train_step(optimizer, params, opt_state_, indices, batch_images, batch_loss)
        if epoch % 200 == 0:
...
if __name__ == "__main__":
    ...
    target_loss_var_ = config["target_loss_var_start"]
    ...
    for epoch in range(config["num_epochs"]):
        ...
        target_loss_var_ *= config["target_loss_var_decay"]

        for batch in range(num_batches):
            key_loss = next(key_gen).get()
            _start = batch * config["batch_size"]
            _end = _start + config["batch_size"]
            _indices = shuffled_indices[_start:_end]
            target_loss = jax.random.normal(key_loss, shape=(len(_indices),), dtype=jnp.float32) * target_loss_var_ 
            params_, opt_state_, _loss_value = train_step(optimizer, params_, opt_state_, _indices, shuffled_images[_start:_end], target_loss)
        ...
        if use_wandb:
            wandb.log({
                ...
                "n_data": n_data_,
                "target_loss_var": target_loss_var_
            })
