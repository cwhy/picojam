...
# Component vocabulary sizes
piece_type_vocab = 7   # 6 piece types + 1 padding
position_vocab = 66    # 64 board + 1 dead + 1 padding  
...
def loss_fn(params, batch_x, batch_y):
    ...
    # Add L2 regularization to all embeddings
    l2_reg = 0.0001 * (
        jnp.sum(params['piece_type_embeddings'] ** 2) +
        jnp.sum(params['position_embeddings'] ** 2) +
...
def test_train_step(params, optimizer, config, key_gen, encoder, num_steps=5):
    ...
    dummy_X = jax.random.randint(key, (10, max_pieces, piece_components), 0, 7)  # Max vocab size for safety
...
if __name__ == "__main__":
    try:
        ...
        logging.info(f"Encoding: Concatenation approach (4 separate embeddings)")
        logging.info(f"Component Vocabs: Piece({piece_type_vocab}), Pos({position_vocab}), Color({color_vocab}), Move({move_vocab})")
