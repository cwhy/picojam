...
def log_embedding_heatmap(image_embeddings, labels, step, num_images=100):
    """Log a heatmap of image embeddings values.
    ...
    # Extract the embeddings for visualization and sort by label
    embeddings = np.array(image_embeddings[:num_images])
    vis_labels = np.array(labels[:num_images])
    
    # Sort by label
    sort_idx = np.argsort(vis_labels)
    embeddings = embeddings[sort_idx]
    vis_labels = vis_labels[sort_idx]
    
    # Create heatmap
    fig, ax = plt.subplots(figsize=(10, 6))
    im = ax.imshow(embeddings, aspect='auto', cmap='viridis')
    
    ax.set_title('Image Embeddings Heatmap (sorted by label)')
    ...
    ax2 = ax.twinx()
    ax2.set_yticks(np.arange(len(vis_labels)))
    if len(vis_labels) <= 20:  # Only show labels if there aren't too many
        ax2.set_yticklabels(vis_labels)
    ...
    # Log to wandb
    wandb.log({"embeddings/heatmap": wandb.Image(Image.open(buf))}, step=step)