...
def load_supervised_image(data: str, n_tr: int | None = None, n_tst: int | None = None) -> ImageClassification:
    ...
    if data == "mnist":
        ...
        mnistData = mnist['train']
        X_img = mnistData['image'][:n_tr] if n_tr else mnistData['image']
        y = mnistData['label'][:n_tr] if n_tr else mnistData['label']
        X_img_test = mnist["test"]["image"][:n_tst] if n_tst else mnist["test"]["image"]
        y_test = mnist["test"]["label"][:n_tst] if n_tst else mnist["test"]["label"]
        n_samples = len(X_img)
        ...
        fashion_mnist = load_dataset("fashion_mnist", cache_dir=cache_dir).with_format("jax")
        fashion_mnistData = fashion_mnist['train']
        X_img = fashion_mnistData['image'][:n_tr] if n_tr else fashion_mnistData['image']
        y = fashion_mnistData['label'][:n_tr] if n_tr else fashion_mnistData['label']
        X_img_test = fashion_mnist["test"]["image"][:n_tst] if n_tst else fashion_mnist["test"]["image"]
        y_test = fashion_mnist["test"]["label"][:n_tst] if n_tst else fashion_mnist["test"]["label"]
        n_samples = len(X_img)
        n_test_samples = len(X_img_test)
        X_train = X_img.reshape((n_samples, 1, 28, 28))
        X_test = X_img_test.reshape((n_test_samples, 1, 28, 28))
        ...
        X_img = cifar10Data['image'][:n_tr] if n_tr else cifar10Data['image']
        y = cifar10Data['label'][:n_tr] if n_tr else cifar10Data['label']
        X_img_test = cifar10["test"]["image"][:n_tst] if n_tst else cifar10["test"]["image"]
        y_test = cifar10["test"]["label"][:n_tst] if n_tst else cifar10["test"]["label"]
        n_samples = len(X_img)
        n_test_samples = len(X_img_test)
